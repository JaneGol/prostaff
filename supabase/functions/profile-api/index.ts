...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (ah) {
      ...
      if (ud?.user) {
        uid = ud.user.id;
        const { data: rs } = await ac
          .from("user_roles")
          .select("role")
          .eq("user_id", uid);
        const ra = (rs || []).map((r: Record<string, string>) => r.role);
        if (ra.includes("admin")) urole = "admin";
        else if (ra.includes("employer")) urole = "employer";
        else if (ra.includes("specialist")) urole = "specialist";
      }
    }
    ...
    if (mode === "single" && pid) {
      return await handleSingle(ac, pid, uid, urole);
    }

    if (mode === "list") {
      return await handleList(ac, urole);
    }
...
async function handleSingle(
  ac: ReturnType<typeof createClient>,
  pid: string,
  uid: string | null,
  urole: string | null,
) {
  const { data: p, error: e } = await ac
    .from("profiles")
  ...
  if (uid && uid === p.user_id) {
    ...
    if (vis === "clubs_only" && urole !== "employer")
      return j({ error: "Not found" }, 404);
    if (urole === "employer" && uid) {
      const { data: ev } = await ac
        .from("profile_views")
        .select("id")
  ...
  const prof: Record<string, unknown> = {
    ...
    is_relocatable: p.is_relocatable,
    is_remote_available: p.is_remote_available,
    role_id: p.role_id,
    secondary_role_id: p.secondary_role_id,
    ...
    hide_current_org: p.hide_current_org,
    ...
    bio: p.bio,
    about_useful: p.about_useful,
    about_style: p.about_style,
    about_goals: p.about_goals,
    email: full ? p.email : null,
    phone: full ? p.phone : null,
    telegram: full ? p.telegram : null,
    linkedin_url: full ? p.linkedin_url : null,
    portfolio_url: full ? p.portfolio_url : null,
    desired_contract_type: p.desired_contract_type,
    desired_city: p.desired_city,
    desired_country: p.desired_country,
    desired_role_ids: p.desired_role_ids,
  };

  let sr = null;
  if (p.secondary_role_id) {
    const { data: srd } = await ac
      .from("specialist_roles")
      .select("id, name")
      .eq("id", p.secondary_role_id)
      .maybeSingle();
    sr = srd;
  }
  ...
  if (access === "preview" && p.hide_current_org) {
    exps = exps.map((x: Record<string, unknown>) => ({
      ...x,
      company_name: x.is_current ? "Hidden org" : x.company_name,
    }));
  }

  let port = r6.data || [];
  if (!full) port = port.filter((x: Record<string, unknown>) => x.visibility === "public");

  return j({
    profile: { ...prof, secondary_role: sr },
    experiences: exps,
    skills: r1.data || [],
    sportsExp: (r2.data || []).map((s: Record<string, unknown>) => ({ ...s, sport: s.sports })),
    sportsOpen: (r3.data || []).map((s: Record<string, unknown>) => ({
      sport_id: s.sport_id,
    ...
    portfolio: port.map((x: Record<string, unknown>) => ({
      ...x,
...
async function handleList(
  ...
  let res = ps || [];
  if (urole !== "employer" && urole !== "admin") {
    res = res.filter((x: Record<string, unknown>) => x.visibility_level !== "clubs_only");
  }

  const san = res.map((x: Record<string, unknown>) => ({
    id: x.id,
    avatar_url: x.avatar_url,
    city: x.city,
    country: x.country,
    level: x.level,
    search_status: x.search_status,
    is_relocatable: x.is_relocatable,
    is_remote_available: x.is_remote_available,
  ...
  if (ids.length > 0) {
    ...
    for (const s of (sd.data || []) as Record<string, unknown>[]) {
      const k = s.profile_id as string;
      if (!psp[k]) psp[k] = [];
      psp[k].push(s);
    }
    for (const s of (kd.data || []) as Record<string, unknown>[]) {
      const k = s.profile_id as string;
      if (!psk[k]) psk[k] = [];
      psk[k].push(s);
    }
