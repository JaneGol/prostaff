...
export function SportsEditor({
  profileId, sportsExperience, sportsOpenTo, onExperienceChange, onOpenToChange,
}: SportsEditorProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-card rounded-xl p-5 md:p-6 shadow-card space-y-4">
        <h2 className="text-[16px] font-semibold text-foreground">Опыт по видам спорта</h2>
        <p className={HINT}>
            ...
            <div key={index} className="border border-border/40 rounded-lg p-3 space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="text-[14px] font-medium truncate text-foreground">{getSportName(exp.sport_id)}</span>
                </div>
              ...
              <div className="flex items-center gap-2">
                <Input
                  type="number" min={1} max={50} value={exp.years}
                  onChange={(e) => updateExperience(index, "years", parseInt(e.target.value) || 1)}
                  className={`w-16 ${FIELD_TEXT}`}
                />
                <span className="text-[13px] text-muted-foreground whitespace-nowrap">лет</span>
                <Select value={exp.level} onValueChange={(v) => updateExperience(index, "level", v)}>
                  <SelectTrigger className={`flex-1 ${FIELD_TEXT}`}>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {levelOptions.map((l) => (
        ...
        {availableSportsForExp.length > 0 && (
          <Select onValueChange={addExperience}>
            <SelectTrigger className={`w-full ${FIELD_TEXT}`}>
      ...
      {/* Open To Sports */}
      <div className="bg-card rounded-xl p-5 md:p-6 shadow-card space-y-4">
        <h2 className="text-[16px] font-semibold text-foreground">Готов работать в</h2>
        <p className={HINT}>
        ...
        <div className="flex flex-wrap gap-2">
          <Badge
            ...
            className="cursor-pointer py-1.5 px-3 transition-colors text-[12px]"
        ...
        <div className="space-y-1.5">
          <Label className={LABEL}>По типу</Label>
          <div className="flex flex-wrap gap-2">
        ...
        <div className="space-y-1.5">
          <Label className={LABEL}>По активности</Label>
          <div className="flex flex-wrap gap-2">
        ...
        {/* Individual sports */}
        <div className="space-y-1.5">
          <Label className={LABEL}>Или конкретные виды</Label>
          <div className="flex flex-wrap gap-2">
            {sportsOpenTo.filter(item => item.sport_id).map((item, index) => {
              ...
              return (
                <Badge
                  key={index}
                  variant="secondary"
                  className="flex items-center gap-1.5 py-1.5 px-3 cursor-pointer hover:bg-destructive/10 text-[12px]"
